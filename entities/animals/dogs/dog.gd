...
signal selected(dog: Dog)
signal care_requested(dog: Dog)

const CARE_DISTANCE_IN_CELLS := 2.0
const CARE_SHAKE_DURATION := 3.0
const CARE_SHAKE_OFFSET := 3.0

## Lógica específica común para todas las razas de perro del refugio.
...
@onready var body_collision_shape: CollisionShape2D = $DogBody/CollisionShape2D
...
func _update_footprint() -> void:
	...
	if visual.texture != null:
		# Cabe en una columna y dos filas sin deformar la ilustración.
		var maximum_size := Vector2(cell_size.x, cell_size.y * 2.0)
		var texture_size := visual.texture.get_size()
		var scale_factor := minf(maximum_size.x / texture_size.x, maximum_size.y / texture_size.y)
		...
		var visual_width := texture_size.x * scale_factor
		visual.position = Vector2(-visual_width * 0.5, cell_size.y * 0.5 - opaque_bottom)
